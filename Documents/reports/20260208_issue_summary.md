...
### B. 操作の継続性
- **現象**: 貼り付けた画像を一度移動させることはできるが、一度マウスを離すと、二回目の移動（ドラッグ）が開始できない。

### C. Undo/Redo の挙動不備
- **現象**: 確定（Commit）操作を行った後の Undo で、確定直前の「選択枠が出ているプレビュー状態」に戻る。
- **期待値**: Undo は「確定（Commit）」というアクション自体を取り消すものであり、確定によって画像が合成される前の状態（貼り付け前、あるいは移動開始前）に画像が戻るべきであり、かつ「確定直前の状態」を再現することが必ずしも適切ではない。
