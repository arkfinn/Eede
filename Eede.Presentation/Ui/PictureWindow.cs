...
namespace Eede.Ui
{
    public partial class PictureWindow : Form
    {
        ...
        public event EventHandler<PicturePushEventArgs> PicturePulled;

        private void InvokePicturePulled(PicturePushEventArgs args)
        {
        ...
        public event EventHandler<PicturePullEventArgs> PicturePushed;

        private void InvokePicturePushed(PicturePullEventArgs args)
        {
        ...
        private void pictureBox1_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            switch (e.Button)
            {
                case MouseButtons.Left:
                    if (!IsSelecting)
                    {
                        InvokePicturePushed(new PicturePullEventArgs(PictureBuffer, CursorPosition.RealPosition));
                        Refresh();
                    }
                    break;
            ...
            switch (e.Button)
            {
                ...
                case MouseButtons.Right:
                    ...
                    if (IsSelecting)
                    {
                        ...
                        Rectangle rect = new(area.X, area.Y, area.Width, area.Height);
                        InvokePicturePulled(new PicturePushEventArgs(PictureBuffer, area));
                        IsSelecting = false;
                    }
                    break;
            }
