...
namespace Eede.Infrastructure.Tests.Launchers;

[TestFixture]
public class ExternalBrowserLauncherTests
{
    private TestExternalBrowserLauncher _launcher;

    private class TestExternalBrowserLauncher : ExternalBrowserLauncher
    {
    ...
    public void Setup()
    {
        _launcher = new TestExternalBrowserLauncher();
    }
    ...
    public void OpenUrl_ShouldNotThrow_AndUrlShouldBeEncoded_WhenUrlIsSafe(string url)
    {
        try
        {
            ...
            Assert.That(_launcher.ProcessStarted, Is.True, "Should trigger process start");
            Assert.That(_launcher.LastUrl, Is.EqualTo(expectedUrl), "Should pass the URL-encoded URL");
        }
    ...
    {
        var ex = Assert.Throws<ArgumentException>(() => _launcher.OpenUrl(url));
