...
public class RegressionTests_ImprovePastePreview
{
    ...
    public async Task Move_Copy_Paste_Commit_ShouldWorkCorrectly()
    {
        ...
        await _viewModel.CopyCommand.Execute().ToTask();
    ...
    [AvaloniaTest]
    public void Cursor_ShouldChange_AfterSelection()
    {
        // 1. (5,5)-(15,15) を選択
        _viewModel.DrawStyle = new RegionSelector();
        _viewModel.DrawBeginCommand.Execute(new Position(5, 5)).Subscribe();
    ...
    [AvaloniaTest]
    public void Cursor_ShouldChange_WhenHoveringSelection()
    {
        // 1. (5,5)-(15,15) を選択
        _viewModel.DrawStyle = new RegionSelector();
        _viewModel.DrawBeginCommand.Execute(new Position(5, 5)).Subscribe();
    ...
    [AvaloniaTest]
    public void Resize_Commit_ShouldWorkCorrectly()
    {
        // Setup: (0,0)-(4,4) に赤い矩形
        var red = new ArgbColor(255, 255, 0, 0);
        var rectData = new byte[4 * 4 * 4];
    ...
    [AvaloniaTest]
    public void Move_Then_Click_Outside_Commits()
    {
        // Setup
    ...
    [AvaloniaTest]
    public void SwitchTool_ShouldCommitAndClearSelection()
    {
        // Setup
    ...
    [AvaloniaTest]
    public void SwitchTool_AfterMoveAndResize_ShouldCommitAndClearSelection()
    {
        // 1. セットアップ
        var picture = Picture.CreateEmpty(new PictureSize(32, 32));
        _sessionProvider.Update(new DrawingSession(picture));
