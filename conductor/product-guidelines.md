...
### 安全なリファクタリング戦略
- **振る舞いテストによる保護:** 大規模なリファクタリングを行う前に、UseCase 等の公開 API に対して現状の挙動を固定する「振る舞いテスト（Characterization Tests）」を作成することで、デグレードを最小限に抑えることができます。
- **Nullable の段階的導入:** 大量の警告が発生しているプロジェクトでは、一度 Nullable を明示的に `disable` にし、テストで保護を固めてから、重要なドメインモデルから順に `enable` に戻してガード節を導入するアプローチが、ノイズに惑わされず着実に品質を向上させるのに有効です。
...
### SVG アイコンの最適化
- **直接埋め込みとスタイル:** アイコンを `StreamGeometry` としてリソース化し、`PathIcon` で表示することで、ビットマップリソースの削減と、拡大耐性、およびテーマ（Foreground）との自動同期を実現します。ファイル分割管理により、複雑なパスデータによるコードの肥大化を防ぎます。
