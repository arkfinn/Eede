...
- **ViewModel 設計:** 肥大化した ViewModel に対しては責務の再配置（Repatriation）を適用し、UI 型に依存しない計算ロジックを Domain 層（Value Object, Entity）や Application 層（UseCase）へ移動することで、テスト可能性と保守性を向上させる。ビットマップリソース（`Bitmap`）に対しては手動での `Dispose()` を徹底し、メモリリークを防止する。
