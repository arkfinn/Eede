...
- [~] Task: `ResizeContext` 値オブジェクトのTDD
    - [ ] **Red**: 比率固定時の連動計算や、px/%の相互変換、アンカーに基づくオフセット計算のテストを記述
    - [ ] **Green**: `ResizeContext` を実装し、テストをパスさせる
    - [ ] **Refactor**: 計算式の整理、VOとしての不変性の確保
- [ ] Task: ニアレストネイバー変換ロジックのTDD
    - [ ] **Red**: 小さなビットマップ（2x2等）を200%, 50%にした際、特定のピクセルが期待通り配置されるかのテストを記述
    - [ ] **Green**: `ImageProcessingHelper` 等に変換ロジックを実装し、テストをパスさせる
    - [ ] **Refactor**: パフォーマンス改善とコードの明瞭化
